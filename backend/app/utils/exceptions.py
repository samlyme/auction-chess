...
class LobbyPermissionError(LobbyError):
    ...
    def __init__(self, user: api.UserProfile, lobby_id: api.LobbyId):
        super().__init__(
            f"User {user} does not have permission to manage Lobby '{lobby_id}'"
        )
        self.detail = {
            "lobby_id": lobby_id,
...
class LobbyCreateError(LobbyError):
    ...
    def __init__(self, user: api.UserProfile, lobby_id: api.LobbyId):
        super().__init__(
            f"User {user} can't create new Lobby because user is already in Lobby '{lobby_id}'"
        )
        self.detail = {
            "lobby_id": lobby_id,
            "user": user.model_dump(mode="json"),
...
class LobbyJoinError(LobbyError):
    ...
    def __init__(self, user: api.UserProfile, lobby_id: api.LobbyId, reason: str = ""):
        super().__init__(
            f"User {user} can't join Lobby '{lobby_id}' because '{reason}'."
        )
        self.detail = {
            "lobby_id": lobby_id,
            "user": user.model_dump(mode="json"),
...
class LobbyLeaveError(LobbyError):
    ...
    def __init__(self, user: api.UserProfile, lobby_id: api.LobbyId, reason: str = ""):
        super().__init__(
            f"User {user} can't leave Lobby '{lobby_id}' because '{reason}'."
        )
        self.detail = {
            "lobby_id": lobby_id,
            "user": user.model_dump(mode="json"),
...
class LobbyStartError(LobbyError):
    ...
    def __init__(self, user: api.UserProfile, lobby_id: api.LobbyId, reason: str = ""):
        super().__init__(
            f"User {user} can't start Lobby '{lobby_id}' because '{reason}'."
        )
        self.detail = {
            "lobby_id": lobby_id,
            "user": user.model_dump(mode="json"),
