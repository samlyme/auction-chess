  ...
  return Object.entries(original).reduce((acc, [key, OriginalRenderFn]) => {
    ...
    acc[key] = (props) => (
      <div className="relative">
        {income !== 0 && (
          <div className="absolute top-0 left-2 text-base text-green-500 [text-shadow:1px_1px_0_rgba(0,0,0,0.8)]">
            +${income}
          </div>
        )}
        {fee !== 0 && (
          <div className="absolute top-0 right-2 text-base text-red-500 [text-shadow:1px_1px_0_rgba(0,0,0,0.8)]">
            -${fee}
          </div>
        )}
